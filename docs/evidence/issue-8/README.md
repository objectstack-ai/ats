...
| `24-shipped-datasets-vs-rest-sqlite.md` | Review follow-up on #54: each of the five SHIPPED datasets queried by `datasetName` (no inline definition) as the job seeker, the platform admin and Quillstone's admin, against the REST count of its base object, sqlite. The seeker agrees on four; `ats_employer_metrics` reads 9 (the seeker's own `seeker_verified_employers` grant; the grouped read is `Verified: 9` and nothing else) where sqlite REST reads 0 (tenant wall). |
| `25-inline-dataset-on-main-sqlite.txt` | The same inline-dataset request on `origin/main` `8d5dcc2` (0 datasets, 0 dashboards, same `requires:`): the seeker's `ats_employer_member` count answers 200 / 24 where REST is 403 — the objectstack#16645 exposure is reachable on `main` today and does not depend on any declared dataset; a `datasetName` lookup there answers 404. |

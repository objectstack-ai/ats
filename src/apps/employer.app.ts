...
/**
 * Hiring — one employer's staff (card 09, DESIGN.md §04). Every entry mounts
 * a view card 07 authored: which employer's rows a person sees is the
 * permission set's row-level rule, never a navigation filter.
 *
...
export const AtsEmployerApp = defineApp({
  name: 'ats_employer_app',
  label: 'Hiring',
  description: "One employer's jobs, pipeline, inbox, interviews and talent pool.",
  icon: 'briefcase',
  branding: { primaryColor: '#0B6E63' },
  // Held only by the two employer permission sets (ats_employer_admin,
  // ats_employer_recruiter).
  requiredPermissions: ['ats_employer.access'],
  navigation: HiringNavigation,
});
